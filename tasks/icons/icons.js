...
const skethcFileName = config.sketchFile // or 'symbol-font-16px.sketch'
  ...
  gulp.src(skethcFileName)
    ...
    .on('glyphs', (glyphs) => {
      ...
      gulp.src(`./src/icons/templates/${template}.css`)
        ...
        .pipe(gulp.dest('./build/icons/css/')) // set path to export your CSS

      // if you don't need sample.html, remove next 4 lines
      gulp.src(`./src/icons/templates/${template}.html`)
        .pipe(consolidate('lodash', options))
        .pipe(rename({ basename: 'sample' }))
        .pipe(gulp.dest('./build/icons/html-layout/')) // set path to export your sample HTML
    })
    .pipe(gulp.dest('./build/icons/fonts')) // set path to export your fonts
)

// gulp.task('watch', ['symbols'], () => {
//   bs.init({
//     files: './build/icons/sample.html',
//     server: './build/icons/',
